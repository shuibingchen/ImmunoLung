...
# load number of captured cells pre-filtering
cell.info <- read.delim(file.path(infodir, 'qc.cell.number.txt'), header=T, 
                        check.names=F, stringsAsFactors=F, sep='\t')
ncells <- cell.info$nPre
names(ncells) <- cell.info$Name

# doublet results
df.results <- data.frame(cellID=character(), orig.ident=character(), 
                         df.isdoublet=character(), df.score=numeric())
...
for (sid in unique(panc$orig.ident)){
  ...
  nExp_poi <- round(doublet.rate*ncells[sid]/1000*ncells[sid])
  ...
  colnames(tdata) <- c('df.score','df.isdoublet')
...
write.table(df.results, file=file.path(infodir, "doublet.results.txt"), 
