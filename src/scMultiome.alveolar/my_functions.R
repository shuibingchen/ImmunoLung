...
my.Read10X.GEX <- function(tcountdir, tprefix=NULL, ftype="Gene Expression"){
  # directory exists?
  ...
  # get GEX data only
  gex.rows <- which(tfnames$V3 == ftype)
  tmat <- tmat[gex.rows,]
  tfnames <- tfnames[gex.rows,]
...
my.compute.QC.metrics <- function(object){
  ...
  object$blacklist_fraction <- FractionCountsInRegion(object=object, 
                                                      assay='ATAC', 
                                                      regions=blacklist.regions)
  return(object)
}

# filter cells that are outliers for these QC metrics.
...
# process ATAC data
my.process.ATAC <- function(object, min.cutoff=20){
  # feature selection
...
my.dot.plot <- function(seurat.obj, genes, cluster.id=0, name.order=c('AVM0','AVM1')){
