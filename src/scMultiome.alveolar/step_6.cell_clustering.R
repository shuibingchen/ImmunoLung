...
panc.rna %<>% FindClusters(resolution=0.65, verbose=T)
...
# Set cell identity
panc.rna %<>% SetIdent(value="RNA_snn_res.0.65")

# Merge clusters
merge.clust <- c(0,0,2,4,5,1,1,3,6,3,2,7,1,0)
names(merge.clust) <- 0:13

final.clust <- as.vector(merge.clust[as.vector(Idents(panc.rna))])
names(final.clust) <- names(Idents(panc.rna))
final.clust <- factor(final.clust, levels=0:7)
